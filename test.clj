...
(defn List'new [s] (&meta &'list (seq s)))
...
(defn String'new [s] (&meta &'string (seq s)))
...
(defn Symbol'new [s] (&meta &'symbol (seq s)))
...
(def Unicode'ζ           (bits '1110110110))
(def Unicode'λ           (bits '1110111011))
...
(defn Compiler'special [s]
    (cond
        (= s 'def)   DefExpr'parse
        (= s 'fn)    FnExpr'parse       (= s 'λ)    FnExpr'parse
        (= s 'if)    IfExpr'parse
...
#_"\n !\"#%&'()*,-./0123456789=>?ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]_abcdefghijklmnopqrstuvwxyzζλ"
